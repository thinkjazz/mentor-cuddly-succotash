...
const ContactGridView: FC = () => {
  const [selectedItem, setSelectedItem] = useState(CONTACT_LIST[1]);
  // search input
  const [searchValue, setSearchValue] = useState("");
  const handleSearch = (value: string) => setSearchValue(value);

  // handle select
  const handleSelectItem = (id: number) => setSelectedItem(CONTACT_LIST[id]);
  // active select item
  const activeItem = (id: number) => selectedItem.id === id;
